...
- 機能ごとに作業ブランチを切る義務 → `develop` への直接 commit / push を許可する
...
- **`main` への直接 push / commit は禁止**。release 時のみ `develop` → `main` の PR を作成する
- hotfix は従来どおり `main` から `hotfix/*` を切って PR で取り込み、同じ変更を `develop` にも反映する
...
- `main` を触る変更（release / hotfix）は必ずオーナーに確認してから進める
