...
- **自動化できる** — CLI から読み込みチェックと PNG 書き出しを実行できる
...
GUI を手で操作せずに、アセットの読み込み確認とスクリーンショット生成を実行できる。CI や納品前チェック、バッチ検証で「開けるか」「最低限描画できるか」を確認する用途を想定している。

| Command              | Purpose                                                                 |
| -------------------- | ----------------------------------------------------------------------- |
| `npm run shot`       | モデルを開き、GUI ビューアと同じ Three.js パイプラインで PNG を書き出す |
| `npm run shot:check` | モデルをロードし、成功 / 失敗を exit code で返す                        |
