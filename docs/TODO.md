...
- [x] 時間表示を実装する
- [x] 複数クリップ選択 UI を実装する
- [ ] 入力データに含まれる場合、アニメーションクリップごとのフレームレート / キーフレーム数を表示する
- [ ] アニメーションインスペクターを強化する（clip duration、track count、target node / bone、property path、keyframe time range、補間方式を確認できる UI）
...
- [x] `Alembic (.abc)` の対応方針を調べる（#69: 同梱 native helper で preview JSON に変換し、静的表示と同一 topology の Geometry Cache を再生）
